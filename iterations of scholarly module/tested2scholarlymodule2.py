"""
This code also is adapted from Greptile's suggestion off tested1scholarlymodule2.py, to really try filtering 
the output of tested1scholarlymodule2.py where, for example, an author "Jules Elkins" can be interpreted as "A Elkins" — just because
the check_name_match function seems to be looking EITHER for "Jules" OR "Elkins".
"""
...
def check_name_match(authors, name):
    name_normalized = name.lower()
    for author in authors:
        if name_normalized in author.lower():
            return True
    return False
